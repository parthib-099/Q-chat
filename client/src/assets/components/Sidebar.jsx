    ...
    return (
        <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? 'max-md-hidden' : ''}`}>
            <div className='pb-5'>
                ...
                <div className='bg-[#282142] rounded-full px-3 py-2 mt-5 flex items-center gap-2'>
            ...
            <div className='flex flex-col '>
                {userDummyData.map((user, index) => (
                    <div onClick={() => setselectedUser(user)}
                        key={index} className={`relative flex items-center gap-3 p-2 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`} >
                        <img src={user?.profilePic || assets.avatar_icon} alt="" className='w-[35px] aspect-[1/1] rounded-full' />
                        <div className='flex flex-col leading-5'>
                            <p>{user.fullName}</p>
                            {
                                index < 3
                                    ? <span className='text-green-500 text-xs'>Online
                                    </span>
